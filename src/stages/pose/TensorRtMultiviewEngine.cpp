...
#include <fstream>
#include <vector>
extern "C" void iris_multiview_preprocess(const void* const*, const std::size_t*, const std::uint32_t*, const std::uint32_t*, const float*, const float*, const float*, float*, cudaStream_t);
...
class TensorRtMultiviewEngine::Impl {
  public:
    explicit Impl(const PoseConfig& config) {
        ...
        validate("images", nvinfer1::Dims5{1, 3, 3, 640, 640});
        validate("R_w2c", nvinfer1::Dims4{1, 3, 3, 3});
        validate("t_w2c", nvinfer1::Dims3{1, 3, 3});
        validate("intrinsics", nvinfer1::Dims4{1, 3, 3, 3});
    ...
    void infer(const std::array<const void*, 3>& sources, const std::array<std::size_t, 3>& strides,
        ...
        check_cuda(cudaMemcpyAsync(k_, calibration_k_.data(), sizeof(float) * 27, cudaMemcpyHostToDevice, stream_), "copy intrinsics");
        ...
        for (std::size_t view = 0; view < 3; ++view) {
            ...
            target_k[view * 9 + 2] = calibration_k_[view * 9 + 2] * scale + (640.0f - widths[view] * scale) * 0.5f;
            target_k[view * 9 + 5] = calibration_k_[view * 9 + 5] * scale + (640.0f - heights[view] * scale) * 0.5f;
        ...
        iris_multiview_preprocess(sources.data(), strides.data(), widths.data(), heights.data(), calibration_k_.data(), target_k.data(), calibration_distortion_.data(), static_cast<float*>(images_), stream_);
        context_->setTensorAddress("images", images_); context_->setTensorAddress("R_w2c", r_); context_->setTensorAddress("t_w2c", t_); context_->setTensorAddress("intrinsics", k_);
        context_->setTensorAddress("poses_3d", poses_); context_->setTensorAddress("joint_valid", valid_); context_->setTensorAddress("joint_scores", scores_);
    ...
    void validate(const char* name, nvinfer1::Dims expected) {
        const auto index = engine_->getTensorIOMode(name);
        if (index == nvinfer1::TensorIOMode::kOUTPUT) throw std::runtime_error(std::string("TensorRT input is output: ") + name);
