...
++published;}catch(const std::exception& e){std::scoped_lock l(lock); error=e.what();}}}); event_worker=std::thread([this]{while(auto packet=event_queue.receive()){std::scoped_lock l(lock); if(!server)continue; if((*packet)->poses)for(const auto& pose:*(*packet)->poses)server->publish_event(pose_event(**packet,pose)); if((*packet)->multiview_poses)server->publish_event(multiview_pose_event(**packet)); }});}
  ...
  void publish(PreviewPacket p) noexcept {if(!running)return; try{std::scoped_lock l(lock);if(config.shared_memory.enabled){const auto result=queue.send(p);if(result==SendResult::ReplacedOldest||result==SendResult::DroppedNewest)++dropped;if(result!=SendResult::Closed)++submitted;}if(mjpeg)mjpeg->publish(p);if(webrtc)webrtc->publish(p);if(server){const auto result=event_queue.send(std::move(p));if(result==SendResult::ReplacedOldest||result==SendResult::DroppedNewest)++event_dropped;}}catch(...){++dropped;}}
  ...
  OutputCommandResult configure_preview(PreviewConfig c){try {if(!output::is_loopback_address(c.http.bind_address))throw std::invalid_argument("preview server must bind to a loopback address");if (c.webrtc.enabled && (!c.webrtc.bitrate || !c.webrtc.max_fps || !c.webrtc.max_width || !c.webrtc.queue_capacity)) throw std::invalid_argument("invalid WebRTC preview configuration");if(running)stop_network();{std::scoped_lock l(lock);config=std::move(c);}if(running)start_network();return {OutputCommandStatus::Applied,"preview configuration applied"};}catch(const std::exception&e){std::scoped_lock l(lock);error=e.what();return {OutputCommandStatus::Failed,error};}}
  void set_status(std::function<std::string()> provider){std::scoped_lock l(lock);status_provider=std::move(provider);}
  PreviewTransportHealth health()const{std::scoped_lock l(lock);auto result=PreviewTransportHealth{config.shared_memory.enabled || config.mjpeg.enabled || config.http.enabled || config.webrtc.enabled,0,published.load(),dropped.load(),error};if(mjpeg){const auto video=mjpeg->health();result.published_packets+=video.published_packets;result.dropped_packets+=video.dropped_packets;if(!video.last_error.empty())result.last_error=video.last_error;}if(webrtc){const auto video=webrtc->health();result.connected_clients=video.connected_clients;result.published_packets+=video.published_packets;result.dropped_packets+=video.dropped_packets;if(!video.last_error.empty())result.last_error=video.last_error;}if(server){auto web=server->health();if(!webrtc)result.connected_clients=web.connected_clients;if(!web.last_error.empty())result.last_error=web.last_error;}return result;}
 private:
  void start_network(){std::scoped_lock l(lock);if(!config.http.enabled)return;if(config.webrtc.enabled){webrtc=std::make_unique<output::WebRtcTransport>(config.webrtc);webrtc->start();}server=std::make_unique<output::PreviewHttpServer>(config.http.bind_address,config.http.port,[this]{return status_provider?status_provider():std::string("{}");},webrtc ? webrtc->signalling_factory() : output::PreviewHttpServer::WebRtcHandlerFactory{});server->start();if(config.mjpeg.enabled){mjpeg=std::make_unique<MjpegTransport>(config.mjpeg,*server);mjpeg->start();}}
  void stop_network() noexcept {std::unique_ptr<MjpegTransport> old_mjpeg;std::unique_ptr<output::WebRtcTransport> old_webrtc;std::unique_ptr<output::PreviewHttpServer> old_server;{std::scoped_lock l(lock);old_mjpeg=std::move(mjpeg);old_webrtc=std::move(webrtc);old_server=std::move(server);}if(old_mjpeg)old_mjpeg->stop();if(old_server)old_server->stop();if(old_webrtc)old_webrtc->stop();}
...
if(c.enabled) throw std::runtime_error("shared-memory output is currently implemented for Windows only");
  ...
  PreviewConfig config; Channel<PreviewPacket> queue,event_queue; std::atomic_bool running{false}; std::thread worker,event_worker; mutable std::mutex lock; std::atomic_size_t submitted{0},published{0},dropped{0},event_dropped{0};std::string error;std::function<std::string()> status_provider;std::unique_ptr<output::PreviewHttpServer> server;std::unique_ptr<MjpegTransport> mjpeg;std::unique_ptr<output::WebRtcTransport> webrtc;
#ifdef _WIN32
