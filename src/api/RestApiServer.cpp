...
json snapshot_pose(const Packet& packet, const std::string& backend) {
    ...
    if (packet.multiview_poses) for (const auto& person : *packet.multiview_poses) { json joints=json::array(), valid=json::array(), predicted=json::array(); for (std::size_t i=0;i<person.joints_3d.size();++i) { const auto& joint=person.joints_3d[i]; joints.push_back({joint[0],joint[1],joint[2]}); valid.push_back(person.joint_valid[i]); predicted.push_back(person.joint_predicted[i]); } pose["multiview"].push_back({{"active",person.active},{"track_id",person.track_id},{"joints3d",joints},{"valid",valid},{"predicted",predicted}}); }
